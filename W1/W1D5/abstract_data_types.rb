...
# are always unique.  When implemented with arrays, a map might look something
# like this: `my_map = [[k1, v1], [k2, v2], [k3, v2], ...]`.
#
# Our Map class should have the following instance methods: `assign(key, value)`,
# `lookup(key)`, `remove(key)`, `show`.  Note that the `assign` method can
# be used to either create a new key-value pair or update the value for a
# pre-existing key.  It's up to you to check whether a key currently exists!
#
# As always, test your code thoroughly to make sure all properties of maps
# are enforced.
